const eulaText = {
    id: `
            <h1>Perjanjian Lisensi Pengguna Akhir (EULA)</h1>
            <p>Efektif per tanggal: 15 Oktober 2024</p>
            <h2>1. Pemberian Lisensi</h2>
            <p>Rajasa Digital memberikan lisensi terbatas, tidak eksklusif, dan tidak dapat dipindahtangankan kepada Anda untuk menggunakan game “Game Console City” (selanjutnya disebut “Game”) di perangkat Anda.</p>
        
            <h2>2. Hak Cipta</h2>
            <p>Game dan semua konten, termasuk tetapi tidak terbatas pada grafik, audio, dan kode sumber, dilindungi oleh undang-undang hak cipta. Anda tidak diperbolehkan untuk mendistribusikan, memodifikasi, atau membuat karya turunan tanpa izin tertulis dari Rajasa Digital.</p>
        
            <h2>3. Penggunaan yang Dilarang</h2>
            <p>Anda setuju untuk tidak menggunakan Game untuk tujuan yang melanggar hukum, merusak, atau menipu. Anda juga tidak diperbolehkan untuk membagikan atau menjual akses ke Game kepada pihak ketiga.</p>
        
            <h2>4. Pembatasan Tanggung Jawab</h2>
            <p>Rajasa Digital tidak bertanggung jawab atas kerusakan langsung, tidak langsung, insidental, atau konsekuensial yang dihasilkan dari penggunaan atau ketidakmampuan untuk menggunakan Game.</p>
        
            <h2>5. Perubahan EULA</h2>
            <p>Rajasa Digital berhak untuk mengubah syarat dan ketentuan dalam EULA ini kapan saja. Perubahan akan diberitahukan kepada Anda melalui aplikasi atau website resmi.</p>
        
            <h2>6. Hukum yang Berlaku</h2>
            <p>EULA ini diatur oleh hukum Republik Indonesia. Setiap perselisihan yang muncul akan diselesaikan di pengadilan yang berwenang di Kota Malang, Jawa Timur, Indonesia.</p>
        
            <h2>7. Persetujuan Anda</h2>
            <p>Dengan mengunduh atau menggunakan Game, Anda menyetujui syarat dan ketentuan dalam EULA ini. Jika Anda tidak setuju, Anda tidak boleh menggunakan Game.</p>
        
            <p>Untuk pertanyaan lebih lanjut, silakan hubungi kami di:</p>
            <p>Email: rajasadigital@gmail.com</p>
    `,
    en: `
        <h1>End User License Agreement (EULA)</h1>
        <p>Effective Date: October 15, 2024</p>

        <h2>1. Grant of License</h2>
        <p>Rajasa Digital grants you a limited, non-exclusive, non-transferable license to use the game "Game Console City" (hereinafter referred to as "Game") on your device.</p>

        <h2>2. Copyright</h2>
        <p>The Game and all content, including but not limited to graphics, audio, and source code, are protected by copyright laws. You may not distribute, modify, or create derivative works without written permission from Rajasa Digital.</p>

        <h2>3. Prohibited Use</h2>
        <p>You agree not to use the Game for any unlawful, harmful, or fraudulent purposes. You are also prohibited from sharing or selling access to the Game to third parties.</p>

        <h2>4. Limitation of Liability</h2>
        <p>Rajasa Digital shall not be liable for any direct, indirect, incidental, or consequential damages arising from the use or inability to use the Game.</p>

        <h2>5. Changes to EULA</h2>
        <p>Rajasa Digital reserves the right to change the terms and conditions in this EULA at any time. Changes will be notified to you through the application or official website.</p>

        <h2>6. Governing Law</h2>
        <p>This EULA is governed by the laws of the Republic of Indonesia. Any disputes arising will be resolved in the competent courts of Malang Regency, East Java.</p>

        <h2>7. Your Acceptance</h2>
        <p>By downloading or using the Game, you agree to the terms and conditions of this EULA. If you do not agree, you may not use the Game.</p>

        <p>For further inquiries, please contact us at:</p>
        <p>Email: rajasadigital@gmail.com</p>
    `,
    th: `
    <h1>ข้อตกลงใบอนุญาตผู้ใช้ปลายทาง (EULA)</h1>
    <p>มีผลตั้งแต่วันที่: 15 ตุลาคม 2024</p>

    <h2>1. การมอบใบอนุญาต</h2>
    <p>Rajasa Digital มอบใบอนุญาตที่จำกัด ไม่สามารถโอน และไม่เป็นเอกสิทธิ์ให้คุณใช้เกม “Game Console City” (ต่อไปนี้เรียกว่า “เกม”) บนอุปกรณ์ของคุณ</p>

    <h2>2. ลิขสิทธิ์</h2>
    <p>เกมและเนื้อหาทั้งหมด รวมถึงแต่ไม่จำกัดเพียง กราฟิก เสียง และรหัสต้นฉบับ ได้รับการคุ้มครองตามกฎหมายลิขสิทธิ์ คุณไม่สามารถแจกจ่าย แก้ไข หรือสร้างผลงานอนุพันธ์โดยไม่ได้รับอนุญาตเป็นลายลักษณ์อักษรจาก Rajasa Digital</p>

    <h2>3. การใช้ที่ต้องห้าม</h2>
    <p>คุณตกลงที่จะไม่ใช้เกมเพื่อวัตถุประสงค์ที่ผิดกฎหมาย เป็นอันตราย หรือหลอกลวง คุณยังไม่สามารถแชร์หรือขายการเข้าถึงเกมให้กับบุคคลที่สาม</p>

    <h2>4. การจำกัดความรับผิดชอบ</h2>
    <p>Rajasa Digital จะไม่รับผิดชอบต่อความเสียหายโดยตรง ไม่โดยตรง อุบัติเหตุ หรือผลที่ตามมาที่เกิดขึ้นจากการใช้หรือไม่สามารถใช้เกมได้</p>

    <h2>5. การเปลี่ยนแปลง EULA</h2>
    <p>Rajasa Digital ขอสงวนสิทธิ์ในการเปลี่ยนแปลงข้อกำหนดและเงื่อนไขใน EULA นี้ได้ตลอดเวลา การเปลี่ยนแปลงจะได้รับการแจ้งให้คุณทราบผ่านทางแอปพลิเคชันหรือเว็บไซต์ทางการ</p>

    <h2>6. กฎหมายที่ใช้บังคับ</h2>
    <p>EULA นี้อยู่ภายใต้กฎหมายของสาธารณรัฐอินโดนีเซีย ข้อพิพาทใดๆ ที่เกิดขึ้นจะต้องได้รับการแก้ไขในศาลที่มีอำนาจในเขตสิงห์อาสา จังหวัดมาลัง</p>

    <h2>7. การยอมรับของคุณ</h2>
    <p>โดยการดาวน์โหลดหรือใช้เกม คุณตกลงตามข้อกำหนดและเงื่อนไขใน EULA นี้ หากคุณไม่เห็นด้วย คุณไม่สามารถใช้เกมได้</p>

    <p>หากต้องการสอบถามข้อมูลเพิ่มเติม กรุณาติดต่อเราที่:</p>
    <p>อีเมล: rajasadigital@gmail.com</p>

    `
};

function changeLanguage() {
    const selectedLanguage = document.getElementById('language').value;
    document.getElementById('eula-text').innerHTML = eulaText[selectedLanguage];
}
